...
var engine = GGANGINAGHAH41A8EH41A8H4D5J1S8JE8HJ41S8H41S8JS87E4HSJLJLJKLHKUH8K1HJ8H8S48SGSDG_jigajgiej_gjiijgsigsgjigjiseji-AFAIGJAIJIGAJIJIGWA545151
    ...
    attachEvents: function() {
        ...
        bot.client.on('myNewBall', function(ball_id) {
            ...
            if (config.verbosityLevel > -1) {
                bot.log('New Thiez Bot Genereted (' + ball_id + ')');
            }
        });
        ...
        bot.client.once('leaderBoardUpdate', function(old, leaders) {
            ...
            if (config.verbosityLevel > 0) {
                bot.log('Leaderboard: ' + name_array.join(' - '));
            }
        });
        ...
        bot.client.on('disconnect', function() {
            if (config.verbosityLevel > 0) {
                bot.log('Thiez Bot Disconnected from the server.');
            }
        ...
        for (var ball_id in bot.client.balls) {
            ...
            if (ball.mine) continue;
              ...
              if (config.botMode == "blind") {
                if(valid_player_pos && valid_player_pos.hasOwnProperty("suicide_targets") && valid_player_pos.suicide_targets == null){
                    console.log("!!UPDATE USERSCRIPT!!")
                    return;
                }

                if(valid_player_pos && valid_player_pos.hasOwnProperty("suicide_targets") && valid_player_pos["suicide_targets"].indexOf(ball.id) > -1){ return ball; }
            } 
...
console.log("Thanks For Download Thiez Bots!");
